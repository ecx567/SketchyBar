...
- [x] 4.1 Rewrite `src/mach.c` — pipe server: `CreateNamedPipe("\\.\pipe\git.felix.<name>", PIPE_ACCESS_DUPLEX|FILE_FLAG_FIRST_PIPE_INSTANCE)`, server thread reads frames, posts `{buffer, MACH_MESSAGE}` to `event_post` — ✅ Pipe server via CreateNamedPipe PIPE_ACCESS_DUPLEX|FILE_FLAG_FIRST_PIPE_INSTANCE; server thread reads NUL-separated frames, posts {buffer, MACH_MESSAGE} to event_post; single-instance with CloseHandle+recreate on client disconnect. platform/win_ipc.h seam header: ipc_frame_valid, ipc_sessions_match, ipc_create_security_descriptor, ipc_client_is_same_session, ipc_pipe_name.
- [x] 4.2 Rewrite `src/mach.c` — `mach_send_message`: open pipe, write NUL-separated argv frame (same as `client_send_message`), read response from `open_memstream` output; timeout=100ms matching macOS — ✅ mach_send_message Windows path: port==NULL→NULL, port==INVALID_HANDLE_VALUE→client (open pipe \\.\pipe\git.felix.<name>, WriteFile+FlushFileBuffers, 100ms byte-mode await_response, timeout→"" via malloc(1)), real handle→write-response-only path (await_response→NULL). Length cap only on client path; responses can exceed 64KB.
- [x] 4.3 Implement IPC frame validation: reject oversized frames (>64KB), malformed NUL separators, connection from non-same-session caller (security descriptor) — ✅ 3-layer security: (1) DACL with SYSTEM S-1-5-18 + current user + logon session SID S-1-5-5-X-Y via ipc_create_security_descriptor, (2) PIPE_REJECT_REMOTE_CLIENTS, (3) post-connect ipc_client_is_same_session via GetNamedPipeClientProcessId→OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION)→OpenProcessToken→TokenSessionId vs ProcessIdToSessionId. ipc_frame_valid rejects >64KB and malformed NUL separators.
- [x] 4.4 Modify `src/sketchybar.c` — lock file (`/tmp/<name>_<user>.lock` + `fcntl`) → named mutex `Local\git.felix.<name>` (`CreateMutex` + `GetLastError==ERROR_ALREADY_EXISTS`) — ✅ acquire_lockfile Windows: SetLastError(0) before CreateMutexA("Local\git.felix.<name>"), capture GetLastError(), CloseHandle on ERROR_ALREADY_EXISTS. advapi32 in default libs.
- [x] 4.5 cmocka test: named-pipe round-trip, timeout, oversized frame rejection, non-session caller rejection — ✅ 6-test cmocka suite (tests/ipc_pipe_test.c): round-trip echo, timeout 300ms→"", oversized 100KB wire + "later requests unaffected", NUL-malformed frames, frame unit validation, session guard with DACL introspection via ConvertSidToStringSidA. Shim (tests/cmocka_shim.{h,c}) under SKBAR_CMOCKA_SHIM for CI without cmocka. CTest target ipc_pipe wired. g_name unique per test (ipc_test_<n>).

## S5: System Events + Message Pump (PR 5)

- [x] 5.1 Create `platform/win_main.c` — `WinMain`: initialize COM, create hidden message-only `HWND`, `GetMessage`/`DispatchMessage` pump, `event_post` from worker threads → `PostMessage(registered_msg)` — ✅ Dedicated pump thread: CoInitializeEx → RegisterClassExW (tolerates ERROR_CLASS_ALREADY_EXISTS) → zero-sized hidden TOP-LEVEL window (WM_DISPLAYCHANGE/WM_POWERBROADCAST only reach top-level windows, not message-only) → InterlockedExchangePointer publishes the HWND → skbar_win_events_init → GetMessageW/DispatchMessageW loop; WM_CLOSE→PostQuitMessage; temp main() under #ifndef SKBAR_NO_MAIN (removed at 7.5). Contract in platform/win_platform.h (skbar_win_is_main_thread, _post_event, _event_hwnd, _handle_message, _monotonic_ns, _animator_timer_id, _animator_timestamp).
- [x] 5.2 Create `platform/win_events.c` — `WM_MOUSEMOVE/WM_LBUTTONUP/WM_MOUSEWHEEL` → hit-test via `bar_manager_get_item_by_point` → `event_post(MOUSE_*)`; `WM_DISPLAYCHANGE` → `event_post(DISPLAY_*)`; `WM_POWERBROADCAST` → `event_post(SYSTEM_WOKE/SYSTEM_WILL_SLEEP)` — ✅ WndProc router: mouse → MOUSE_* (click/scroll cross the queue by-point; enter/exit route directly to bar_manager_handle_mouse_entered/exited until S6 real windows), WM_TIMER(animator id) → ANIMATOR_REFRESH with QPC ticks, WM_DISPLAYCHANGE → DISPLAY_CHANGED, WM_POWERBROADCAST → SYSTEM_WILL_SLEEP/SYSTEM_WOKE, WM_APP_EVENT → marshalled event_post copy (heap, pump frees).
- [x] 5.3 Rewrite `src/animation.c` — 60Hz `SetTimer`/`QueryPerformanceCounter` tick replaces `CVDisplayLink`; `ANIMATOR_REFRESH` dispatched on timer — ✅ SetTimer((HWND)hwnd,(UINT_PTR)animator,16,NULL) beat + QPC frequency/timestamp into the refresh context; timer_id uintptr_t under #ifdef _WIN32; macOS CVDisplayLink body preserved byte-identical under #else.
- [x] 5.4 Rewrite `src/mouse.c` — remove Carbon kEventClassMouse; mouse events now routed from `platform/win_events.c` WM_* handlers — ✅ Windows mouse_begin no-op; Carbon spec/InstallEventHandler body preserved under #else (only the Carbon include moved inside the guard).
- [x] 5.5 Rewrite `src/hotload.c` — `ReadDirectoryChangesW` thread watches config dir; 500ms rate limiter preserved; `fork_exec --reload` → `CreateProcess` with sh — ✅ config_watch_thread: FILE_LIST_DIRECTORY + FILE_FLAG_OVERLAPPED, RDCW 64KB buffer, WaitForMultipleObjects on {overlapped.hEvent, stop}, 500ms HOTLOAD_DEBOUNCE_MS gate → event_post(HOTLOAD); bounded teardown (SetEvent + 2s join); non-destructive win_dirname; FSEvents body preserved under #else. Real CreateProcess runner deferred to 7.6 (S1 fork_exec shim retained).
- [x] 5.6 Modify `src/event.c` — `event_post` uses `PostMessage(hwnd, WM_APP_EVENT, ...)` instead of `dispatch_async`; main pump dispatches `bar_manager_handle_*` — ✅ ONLY event_post got the Windows branch (lines 386-392): main thread → inline event_execute; off-main → skbar_win_post_event heap copy + PostMessage(WM_APP_EVENT). Shared event_handler[] dispatch table untouched; macOS branch byte-identical.
- [x] 5.7 cmocka test: event dispatch, mouse hit-test, config dir change triggers HOTLOAD — ✅ tests/events_test.c: 5 deterministic cmocka tests (worker-thread marshal, mouse routing/hover, animator SetTimer beat + QPC, live RDCW hotload + 500ms debounce, display/power); stub bar_manager surface pinned to real bar_manager.h signatures; `events` ctest target mirrors the ipc_pipe wiring. Evidence: 100% tests passed (5/5), 8/8 consecutive runs.
